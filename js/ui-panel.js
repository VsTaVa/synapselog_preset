...
// (3개도 해봤지만 경계가 둘이라 비율 드래그를 못 쓰고 3등분 고정이 돼, 2개 유지)
const MAX_STACK = 2;
...
function _makeGhosts(wrap) {
  ...
  Array.from(wrap.children).forEach(el => {
    ...
    ghost.removeAttribute('data-nid'); ghost.removeAttribute('data-pane');
...
function toggleDetailSettings(anchor, i, n, notionHref) {
  ...
  const html = `<div class="menu-head"><span>메뉴</span><button type="button" class="ui-help-btn${_dsHelpOpen ? ' on' : ''}" onclick="toggleDsHelp(this)" title="각 항목 설명 보기" aria-label="각 항목 설명 보기">${infoIcon(13)}</button></div>`
