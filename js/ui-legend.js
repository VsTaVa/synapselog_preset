...
// 위성 모드 — 앱 로고(8각별). 그래프 최상위 노드와 같은 drawStar8로 그려 모양이 갈라지지 않는다.
// 색은 CSS가 입힌다(마스크) → 이미지로 박아 넣는 것과 달리 버튼 호버 색을 그대로 따라간다
let _logoMaskUrl = '';
function logoIcon(px) {
  if (!_logoMaskUrl) {
    const S = 64, c = document.createElement('canvas'); c.width = S; c.height = S;
    const g = c.getContext('2d');
    drawStar8(g, S/2, S/2, S/4); g.fillStyle = '#000'; g.fill(); // 마스크라 색은 의미 없음
    _logoMaskUrl = c.toDataURL();
  }
  return `<span class="mask-ic" style="width:${px}px;height:${px}px;-webkit-mask-image:url(${_logoMaskUrl});mask-image:url(${_logoMaskUrl})"></span>`;
}
...
function _exploreToolsHtml() {
  ...
  html += `<button onclick="multiSelectSatellite()" title="선택한 노드와 하위 노드를 상위에서 분리/복원">${logoIcon(13)} 위성 모드${satOn ? ' 해제' : ''}</button>`;
